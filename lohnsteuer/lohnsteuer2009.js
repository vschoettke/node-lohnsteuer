...
module.exports = function Lohnsteuer2009Big(args) {
    //  Stand: 2015-11-16
    //  ZIVIT Düsseldorf
    //   EINGABEPARAMETER
    ...
    function MZTABFB() {
        ...
        if (STKL === 1) {
            ...
            KFB = (ZKF.multiply(new BigDecimal("6024"))).round(0, BigDecimal.RoundingModes.DOWN);
        } else {
            if (STKL === 2) {
                ...
                if (STKL === 3) {
                    ...
                    if (STKL === 4) {
                        SAP = new BigDecimal("36");
                        KFB = (ZKF.multiply(new BigDecimal("3012"))).round(0, BigDecimal.RoundingModes.DOWN);
                    } else {
    ...
    function UPEVP() {
        ...
        if (KRV > 0) {
            ...
            if (ZRE4VP.compareTo(new BigDecimal("64800")) === 1) {
                ZRE4VP = new BigDecimal("64800");
            }
            VSP1 = (ZRE4VP.multiply(new BigDecimal("0.36"))).round(2, BigDecimal.RoundingModes.DOWN);
    ...
    function MST5_6() {
        ZZX = X;
        if (ZZX.compareTo(new BigDecimal("26276")) === 1) {
            ZX = new BigDecimal("26276");
            UP5_6();
            if (ZZX.compareTo(new BigDecimal("200320")) === 1) {
                ST = (ST.add((new BigDecimal("200320").subtract(new BigDecimal("26276"))).multiply(new BigDecimal("0.42")))).round(0, BigDecimal.RoundingModes.DOWN);
                ST = (ST.add((ZZX.subtract(new BigDecimal("200320"))).multiply(new BigDecimal("0.45")))).round(0, BigDecimal.RoundingModes.DOWN);
            } else {
                ST = (ST.add((ZZX.subtract(new BigDecimal("26276"))).multiply(new BigDecimal("0.42")))).round(0, BigDecimal.RoundingModes.DOWN);
            }
        } else {
            ZX = ZZX;
            UP5_6();
            if (ZZX.compareTo(new BigDecimal("9225")) === 1) {
                VERGL = ST;
                ZX = new BigDecimal("9225");
                UP5_6();
                HOCH = (ST.add((ZZX.subtract(new BigDecimal("9225"))).multiply(new BigDecimal("0.42")))).round(0, BigDecimal.RoundingModes.DOWN);
    ...
    function UPTAB09() {
        if (X.compareTo(new BigDecimal("7835")) === -1) {
            ST = new BigDecimal("0");
        } else {
            if (X.compareTo(new BigDecimal("13140")) === -1) {
                Y = (X.subtract(new BigDecimal("7834"))).divide(new BigDecimal("10000"), 6, BigDecimal.ROUND_DOWN);
                RW = Y.multiply(new BigDecimal("939.68"));
                RW = RW.add(new BigDecimal("1400"));
                ST = (RW.multiply(Y)).round(0, BigDecimal.RoundingModes.DOWN);
            } else {
                if (X.compareTo(new BigDecimal("52552")) === -1) {
                    Y = (X.subtract(new BigDecimal("13139"))).divide(new BigDecimal("10000"), 6, BigDecimal.ROUND_DOWN);
                    RW = Y.multiply(new BigDecimal("228.74"));
                    RW = RW.add(new BigDecimal("2397"));
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("1007"))).round(0, BigDecimal.RoundingModes.DOWN);
                } else {
                    if (X.compareTo(new BigDecimal("250401")) === -1) {
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("8064"))).round(0, BigDecimal.RoundingModes.DOWN);
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("15576"))).round(0, BigDecimal.RoundingModes.DOWN);
                    }
                }
            }
        }

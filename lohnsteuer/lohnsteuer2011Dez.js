...
module.exports = function Lohnsteuer2011Big(args) {
    //  Stand:  2015-11-16
    //  ZIVIT Düsseldorf
    //   EINGABEPARAMETER
    ...
    var RENTBEMESSUNGSGR_OST_2011 = new BigDecimal("57600"); // BigDecimal

    //  Rentenbemessungs-Grenze alte Bundesländer in EUR
    var RENTBEMESSUNGSGR_WEST = new BigDecimal("66000"); // BigDecimal
    ...
    function UPANP12() {
        if (LZZ === 1) {
            if ((ZRE4.subtract(ZVBEZ)).compareTo(new BigDecimal("1000")) === -1) {
                ANP = ((ANP.add(ZRE4)).subtract(ZVBEZ)).round(0, BigDecimal.RoundingModes.UP);
            } else {
                ANP = ANP.add(new BigDecimal("1000"));
            }
        } else {
            if ((ZRE4.subtract(ZVBEZ)).compareTo(new BigDecimal("1880")) === -1) {
                ANP = ((ANP.add(ZRE4)).subtract(ZVBEZ)).round(0, BigDecimal.RoundingModes.UP);;
            } else {
                ANP = ANP.add(new BigDecimal("1880"));
            }
        }
    }
    ...
    function UPEVP() {
        if (KRV > 1) {
            ...
            VSP1 = (ZRE4VP.multiply(new BigDecimal("0.44"))).round(2, BigDecimal.RoundingModes.DOWN);
    ...
    function MVSP() {
        if (ZRE4VP.compareTo(new BigDecimal("44550")) === 1) {
            ZRE4VP = new BigDecimal("44550");
        }
        if (PKV > 0) {
            if (STKL === 6) {
                ...
                if (PKV === 2) {
                    ...
                    VSP = VSP.subtract(ZRE4VP.multiply(KVSATZAG.add(PVSATZAG))).round(2, BigDecimal.RoundingModes.DOWN);
                }
            }
        } else {
            ...
            VSP = ZRE4VP.multiply(KVSATZAN.add(PVSATZAN)).round(2, BigDecimal.RoundingModes.DOWN);
        }
        VSP = VSP.add(VSP1).round(0, BigDecimal.RoundingModes.UP);
    }

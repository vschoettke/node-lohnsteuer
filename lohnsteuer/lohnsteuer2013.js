...
// const BigDecimal
var RENTBEMESSUNGSGR_OST_2013 = new BigDecimal("58800");
// const BigDecimal
var RENTBEMESSUNGSGR_WEST = new BigDecimal("69600");
module.exports = function Lohnsteuer2013Big(args) {
    //  Stand: 2015-11-16
    //  ZIVIT Düsseldorf
    //   EINGABEPARAMETER
    ...
    //                   a) Es wird davon ausge-gangen, dassa) für die BBG (Ost) 58.800 Euro und für die BBG (West) 69.600 Euro festgelegt wird sowie
    ...
    function UPEVP() {
        if (KRV > 1) {
            ...
            //  Neuer Wert 2013
            VSP1 = (ZRE4VP.multiply(new BigDecimal("0.52"))).round(2, BigDecimal.RoundingModes.DOWN);
    ...
    function MST5_6() {
        ...
        if (ZZX.compareTo(new BigDecimal("26441")) === 1) {
            ...
            if (ZZX.compareTo(new BigDecimal("9550")) === 1) {
                ...
                UP5_6();
                HOCH = (ST.add((ZZX.subtract(new BigDecimal("9550"))).multiply(new BigDecimal("0.42")))).round(0, BigDecimal.RoundingModes.DOWN);
        ...
        if (X.compareTo(new BigDecimal("8131")) === -1) {
            ...
            if (X.compareTo(new BigDecimal("13470")) === -1) {
                ...
                if (X.compareTo(new BigDecimal("52882")) === -1) {
                    ...
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("1014"))).round(0, BigDecimal.RoundingModes.DOWN);
                } else {
                    if (X.compareTo(new BigDecimal("250731")) === -1) {
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("8196"))).round(0, BigDecimal.RoundingModes.DOWN);
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("15718"))).round(0, BigDecimal.RoundingModes.DOWN);
                    }
                }
            }
        }

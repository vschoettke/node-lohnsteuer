...
module.exports = function Lohnsteuer2016Big(args) {
    //  Stand: 2015-11-06
    //  ZIVIT Düsseldorf
    //   EINGABEPARAMETER
        ...
        if (ZKF.compareTo(new BigDecimal("0")) === 1)         //  ZKF > 0
{
    ...
    function MZTABFB() {
        ...
        if (STKL === 1) {
            ...
            KFB = (ZKF.multiply(new BigDecimal("7248"))).round(0, BigDecimal.RoundingModes.DOWN);
            //  Neu 2016
        } else {
            if (STKL === 2) {
                ...
                if (STKL === 3) {
                    ...
                    if (STKL === 4) {
                        SAP = new BigDecimal("36");
                        KFB = (ZKF.multiply(new BigDecimal("3624"))).round(0, BigDecimal.RoundingModes.DOWN);
                        //  Neu 2016
                    } else {
                        if (STKL === 5) {
                            SAP = new BigDecimal("36");
        ...
        if (X.compareTo(GFB.add(ZAHL1)) === -1) {
            ...
            if (X.compareTo(new BigDecimal("13670")) === -1) {
                ...
                if (X.compareTo(new BigDecimal("53666")) === -1) {
                    Y = (X.subtract(new BigDecimal("13669"))).divide(ZAHL10000).round(6, BigDecimal.RoundingModes.DOWN);
                    RW = Y.multiply(new BigDecimal("225.40"));
                    RW = RW.add(new BigDecimal("2397"));
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("952.48"))).round(0, BigDecimal.RoundingModes.DOWN);
                } else {
                    if (X.compareTo(new BigDecimal("254447")) === -1) {
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("8394.14"))).round(0, BigDecimal.RoundingModes.DOWN);
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("16027.52"))).round(0, BigDecimal.RoundingModes.DOWN);
                    }
                }
            }
        }

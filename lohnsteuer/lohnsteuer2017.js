...
module.exports = function Lohnsteuer2017Big(args) {
    //  Stand: 2016-11-08
    //  ITZBund Düsseldorf
    //   EINGABEPARAMETER
...
{
            if (KRV === 0) {
                BBGRV = new BigDecimal("76200");
                //  Neu 2017
            } else {
                BBGRV = new BigDecimal("68400");
                //  Neu 2017
            }
            RVSATZAN = new BigDecimal("0.0935");
            TBSVORV = new BigDecimal("0.68");
            //  Neu 2017
        } else {
            //  Nichts zu tun
        }
        BBGKVPV = new BigDecimal("52200");
        //  Neu 2017
        KVSATZAN = (KVZ.divide(ZAHL100)).add(new BigDecimal("0.07"));
        KVSATZAG = new BigDecimal("0.07");
        if (PVS === 1) {
            PVSATZAN = new BigDecimal("0.01775");
            //  Neu 2017
            PVSATZAG = new BigDecimal("0.00775");
            //  Neu 2017
        } else {
            PVSATZAN = new BigDecimal("0.01275");
            //  Neu 2017
            PVSATZAG = new BigDecimal("0.01275");
            //  Neu 2017
        }
        if (PVZ === 1) {
        ...
        if (ZKF.compareTo(new BigDecimal("0")) === 1)         //  ZKF > 0
{
    ...
    function MZTABFB() {
        ...
        if (STKL === 1) {
            ...
            KFB = (ZKF.multiply(new BigDecimal("7356"))).round(0, BigDecimal.RoundingModes.DOWN);
            //  Neu 2017
        } else {
            if (STKL === 2) {
                ...
                if (STKL === 3) {
                    ...
                    if (STKL === 4) {
                        SAP = new BigDecimal("36");
                        KFB = (ZKF.multiply(new BigDecimal("3678"))).round(0, BigDecimal.RoundingModes.DOWN);
                        //  Neu 2017
                    } else {
                        if (STKL === 5) {
                            SAP = new BigDecimal("36");
            ...
            if (WVFRBM.compareTo(new BigDecimal("0")) === -1)             //  WVFRBM < 0
{
    ...
    function UPTAB17() {
        if (X.compareTo(GFB.add(ZAHL1)) === -1) {
            ...
            if (X.compareTo(new BigDecimal("13770")) === -1) {
                Y = (X.subtract(GFB)).divide(ZAHL10000).round(6, BigDecimal.RoundingModes.DOWN);
                RW = Y.multiply(new BigDecimal("1007.27"));
                RW = RW.add(new BigDecimal("1400"));
                ST = (RW.multiply(Y)).round(0, BigDecimal.RoundingModes.DOWN);
            } else {
                if (X.compareTo(new BigDecimal("54058")) === -1) {
                    Y = (X.subtract(new BigDecimal("13769"))).divide(ZAHL10000).round(6, BigDecimal.RoundingModes.DOWN);
                    RW = Y.multiply(new BigDecimal("223.76"));
                    RW = RW.add(new BigDecimal("2397"));
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("939.57"))).round(0, BigDecimal.RoundingModes.DOWN);
                } else {
                    if (X.compareTo(new BigDecimal("256304")) === -1) {
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("8475.44"))).round(0, BigDecimal.RoundingModes.DOWN);
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("16164.53"))).round(0, BigDecimal.RoundingModes.DOWN);
                    }
                }
            }
        }

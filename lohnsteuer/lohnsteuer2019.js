...
module.exports = function Lohnsteuer2019Big(args) {
    //  Stand: 2018-11-12
    //  ITZBund Düsseldorf
    //   EINGABEPARAMETER
    ...
    var bd = new BigDecimal("0"); // BigDecimal
        ...
        if (KRV < 2)         //  &lt; = <
{
            if (KRV === 0) {
                ...
                BBGRV = new BigDecimal("73800");
                //  Neu 2019
            }
            RVSATZAN = new BigDecimal("0.093");
            //  Neu 2019
            TBSVORV = new BigDecimal("0.76");
            //  Neu 2019
        } else {
            //  Nichts zu tun
        }
        BBGKVPV = new BigDecimal("54450");
        //  Neu 2019
        bd = new BigDecimal("2");
        //  Neu 2019
        KVSATZAN = (KVZ.divide(bd).divide(ZAHL100)).add(new BigDecimal("0.07"));
        //  Neu 2019
        KVSATZAG = new BigDecimal(String(0.0045+0.07));
        //  Neu 2019
        if (PVS === 1) {
        ...
        //  Anfang Neu 2019
        W1STKL5 = new BigDecimal("10635");
        W2STKL5 = new BigDecimal("27980");
        W3STKL5 = new BigDecimal("212261");
        GFB = new BigDecimal("9168");
        //  Ende Neu 2019
        ...
        if (ZKF.compareTo(new BigDecimal("0")) === 1)         //  ZKF > 0
{
    ...
    function MZTABFB() {
        ...
        if (STKL === 1) {
            ...
            KFB = (ZKF.multiply(new BigDecimal("7620"))).round(0, BigDecimal.RoundingModes.DOWN);
            //  Neu 2019
        } else {
            if (STKL === 2) {
                ...
                if (STKL === 3) {
                    ...
                    if (STKL === 4) {
                        SAP = new BigDecimal("36");
                        KFB = (ZKF.multiply(new BigDecimal("3810"))).round(0, BigDecimal.RoundingModes.DOWN);
                        //  Neu 2019
                    } else {
                        if (STKL === 5) {
                            SAP = new BigDecimal("36");
            ...
            if (WVFRBM.compareTo(new BigDecimal("0")) === -1)             //  WVFRBM < 0
{
    ...
    function UPTAB19() {
        if (X.compareTo(GFB.add(ZAHL1)) === -1) {
            ...
            if (X.compareTo(new BigDecimal("14255")) === -1) {
                Y = (X.subtract(GFB)).divide(ZAHL10000).round(6, BigDecimal.RoundingModes.DOWN);
                RW = Y.multiply(new BigDecimal("980.14"));
                RW = RW.add(new BigDecimal("1400"));
                ST = (RW.multiply(Y)).round(0, BigDecimal.RoundingModes.DOWN);
            } else {
                if (X.compareTo(new BigDecimal("55961")) === -1) {
                    Y = (X.subtract(new BigDecimal("14254"))).divide(ZAHL10000).round(6, BigDecimal.RoundingModes.DOWN);
                    RW = Y.multiply(new BigDecimal("216.16"));
                    RW = RW.add(new BigDecimal("2397"));
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("965.58"))).round(0, BigDecimal.RoundingModes.DOWN);
                } else {
                    if (X.compareTo(new BigDecimal("265327")) === -1) {
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("8780.9"))).round(0, BigDecimal.RoundingModes.DOWN);
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("16740.68"))).round(0, BigDecimal.RoundingModes.DOWN);
                    }
                }
            }
        }

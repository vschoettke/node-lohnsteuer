...
module.exports = function Lohnsteuer2020Big(args) {
    //  Stand: 2019-11-29
    //  ITZBund Düsseldorf
    //   EINGABEPARAMETER
...
{
            if (KRV === 0) {
                BBGRV = new BigDecimal("82800");
                //  Neu 2020
            } else {
                BBGRV = new BigDecimal("77400");
                //  Neu 2020
            }
            RVSATZAN = new BigDecimal("0.093");
            //  Neu 2019
            TBSVORV = new BigDecimal("0.8");
            //  Neu 2020
        } else {
            //  Nichts zu tun
        }
        BBGKVPV = new BigDecimal("56250");
        //  Neu 2020
        bd = new BigDecimal("2");
        //  Neu 2019
        KVSATZAN = (KVZ.divide(bd).divide(ZAHL100)).add(new BigDecimal("0.07"));
        //  Neu 2019
        KVSATZAG = new BigDecimal(String(0.0055+0.07));
        //  Neu 2020
        if (PVS === 1) {
        ...
        if (ZKF.compareTo(new BigDecimal("0")) === 1)         //  ZKF > 0
{
    ...
    function MZTABFB() {
        ...
        if (STKL === 1) {
            ...
            KFB = (ZKF.multiply(new BigDecimal("7812"))).round(0, BigDecimal.RoundingModes.DOWN);
            //  Neu 2020
        } else {
            if (STKL === 2) {
                ...
                if (STKL === 3) {
                    ...
                    if (STKL === 4) {
                        SAP = new BigDecimal("36");
                        KFB = (ZKF.multiply(new BigDecimal("3906"))).round(0, BigDecimal.RoundingModes.DOWN);
                        //  Neu 2020
                    } else {
                        if (STKL === 5) {
                            SAP = new BigDecimal("36");
            ...
            if (WVFRBM.compareTo(new BigDecimal("0")) === -1)             //  WVFRBM < 0
{
    ...
    function UPTAB20() {
        if (X.compareTo(GFB.add(ZAHL1)) === -1) {
            ...
            if (X.compareTo(new BigDecimal("14533")) === -1)             //  neu seit 2020
{
                Y = (X.subtract(GFB)).divide(ZAHL10000).round(6, BigDecimal.RoundingModes.DOWN);
                RW = Y.multiply(new BigDecimal("972.87"));
                //  neu seit 2020
                RW = RW.add(new BigDecimal("1400"));
                ST = (RW.multiply(Y)).round(0, BigDecimal.RoundingModes.DOWN);
            } else {
                if (X.compareTo(new BigDecimal("57052")) === -1)                 //  neu seit 2020
{
                    Y = (X.subtract(new BigDecimal("14532"))).divide(ZAHL10000).round(6, BigDecimal.RoundingModes.DOWN);
                    RW = Y.multiply(new BigDecimal("212.02"));
                    //  neu seit 2020
                    RW = RW.add(new BigDecimal("2397"));
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("972.79"))).round(0, BigDecimal.RoundingModes.DOWN);
                    //  neu seit 2020
                } else {
                    if (X.compareTo(new BigDecimal("270501")) === -1)                     //  neu seit 2020
{
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("8963.74"))).round(0, BigDecimal.RoundingModes.DOWN);
                        //  neu seit 2020
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("17078.74"))).round(0, BigDecimal.RoundingModes.DOWN);
                        //  neu seit 2020
                    }
                }
            }

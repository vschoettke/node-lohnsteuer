...
{
                    Y = (X.subtract(new BigDecimal("14753"))).divide(ZAHL10000).round(6, BigDecimal.RoundingModes.DOWN);
                    //  Geändert für 2021
                    RW = Y.multiply(new BigDecimal("208.85"));
                    //  Geändert für 2021
                    RW = RW.add(new BigDecimal("2397"));
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("950.96"))).round(0, BigDecimal.RoundingModes.DOWN);
                    //  Geändert für 2021
                } else {
                    if (X.compareTo(new BigDecimal("274613")) === -1)                     //  Geändert für 2021
{
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("9136.63"))).round(0, BigDecimal.RoundingModes.DOWN);
                        //  Geändert für 2021
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("17374.99"))).round(0, BigDecimal.RoundingModes.DOWN);
                        //  Geändert für 2021
                    }
                }
            }

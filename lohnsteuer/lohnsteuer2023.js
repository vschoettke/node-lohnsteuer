...
module.exports = function Lohnsteuer2023Big(args) {
    ...
    // BigDecimal - Implicit Default
    var MBV = (args.MBV !== undefined) ? new BigDecimal(String(args.MBV)) : BigDecimal.ZERO;
        ...
        if (KRV < 2)         //  &lt; = <
{
        ...
        //  Neu 2019
        KVSATZAG = new BigDecimal(String(0.008+0.07));
        //  Geändert für 2023
        if (PVS === 1) {
        ...
        if (PVZ === 1) {
            PVSATZAN = PVSATZAN.add(new BigDecimal("0.0035"));
            //   geändert für 2022
        }
    ...
    function MZTABFB() {
        ...
        if (STKL < 6) {
            if (ZRE4.compareTo(ZVBEZ) === 1) {
                if (ZRE4.subtract(ZVBEZ).compareTo(new BigDecimal("1200")) === -1) {
                    ANP = ANP.add(ZRE4).subtract(ZVBEZ).setScale(0, BigDecimal.ROUND_UP);
                } else {
                    ANP = ANP.add(new BigDecimal("1200"));
                }
            }
        }
        KZTAB = 1;
        if (STKL === 1) {
            ...
            KFB = (ZKF.multiply(new BigDecimal("8952"))).setScale(0, BigDecimal.ROUND_DOWN);
            //  Geändert für 2023
        } else {
            if (STKL === 2) {
                EFA = new BigDecimal("4008");
                //  Geändert für 2022
                SAP = new BigDecimal("36");
                KFB = (ZKF.multiply(new BigDecimal("8952"))).setScale(0, BigDecimal.ROUND_DOWN);
                //  Geändert für 2023
                ...
                if (STKL === 3) {
                    ...
                    if (STKL === 4) {
                        SAP = new BigDecimal("36");
                        KFB = (ZKF.multiply(new BigDecimal("4476"))).setScale(0, BigDecimal.ROUND_DOWN);
                        //  Geändert für 2023
                    } else {
                        if (STKL === 5) {
                            SAP = new BigDecimal("36");
...
{
            if (MBV.compareTo(BigDecimal.ZERO) === 0)             //   MBV = 0
{
                //  absichtlich leer
            } else {
                ...
                if (LSTLZZ.compareTo(BigDecimal.ZERO) === -1)                 //   LSTLZZ < 0
{
                ...
                SOLZLZZ = SOLZLZZ.add(STS.multiply(new BigDecimal("5.5").divide(ZAHL100))).setScale(0, BigDecimal.ROUND_DOWN);
            ...
            if (SOLZVBMG.compareTo(SOLZFREI) === 1)             //  SOLZVBMG > SOLZFREI
{
    ...
    function UPTAB23() {
        ...
        if (X.compareTo(GFB.add(ZAHL1)) === -1) {
            ...
            if (X.compareTo(new BigDecimal("16000")) === -1)             //  Geändert für 2023
{
                Y = (X.subtract(GFB)).divide(ZAHL10000, 6, BigDecimal.ROUND_DOWN);
                RW = Y.multiply(new BigDecimal("979.18"));
                //  Geändert für 2023
                RW = RW.add(new BigDecimal("1400"));
                ST = (RW.multiply(Y)).setScale(0, BigDecimal.ROUND_DOWN);
            } else {
                if (X.compareTo(new BigDecimal("62810")) === -1)                 //  Geändert für 2023
{
                    Y = (X.subtract(new BigDecimal("15999"))).divide(ZAHL10000, 6, BigDecimal.ROUND_DOWN);
                    //  Geändert für 2023
                    RW = Y.multiply(new BigDecimal("192.59"));
                    //  Geändert für 2023
                    RW = RW.add(new BigDecimal("2397"));
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("966.53"))).setScale(0, BigDecimal.ROUND_DOWN);
                    //  Geändert für 2023
                } else {
                    if (X.compareTo(new BigDecimal("277826")) === -1)                     //  Geändert für 2022
{
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("9972.98"))).setScale(0, BigDecimal.ROUND_DOWN);
                        //  Geändert für 2023
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("18307.73"))).setScale(0, BigDecimal.ROUND_DOWN);
                        //  Geändert für 2023
                    }
                }
            }

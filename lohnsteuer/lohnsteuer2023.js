        ...
        if (ZKF.compareTo(new BigDecimal("0")) === 1)         //  ZKF > 0
{
    ...
    function MZTABFB() {
        ...
        if (STKL === 1) {
            ...
            KFB = (ZKF.multiply(new BigDecimal("8952"))).round(0, BigDecimal.RoundingModes.DOWN);
            //  Geändert für 2023
        } else {
            if (STKL === 2) {
                ...
                if (STKL === 3) {
                    ...
                    if (STKL === 4) {
                        SAP = new BigDecimal("36");
                        KFB = (ZKF.multiply(new BigDecimal("4476"))).round(0, BigDecimal.RoundingModes.DOWN);
                        //  Geändert für 2023
                    } else {
                        if (STKL === 5) {
                            SAP = new BigDecimal("36");
...
{
                    Y = (X.subtract(new BigDecimal("15999"))).divide(ZAHL10000).round(6, BigDecimal.RoundingModes.DOWN);
                    //  Geändert für 2023
                    RW = Y.multiply(new BigDecimal("192.59"));
                    //  Geändert für 2023
                    RW = RW.add(new BigDecimal("2397"));
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("966.53"))).round(0, BigDecimal.RoundingModes.DOWN);
                    //  Geändert für 2023
                } else {
                    if (X.compareTo(new BigDecimal("277826")) === -1)                     //  Geändert für 2022
{
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("9972.98"))).round(0, BigDecimal.RoundingModes.DOWN);
                        //  Geändert für 2023
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("18307.73"))).round(0, BigDecimal.RoundingModes.DOWN);
                        //  Geändert für 2023
                    }
                }
            }
